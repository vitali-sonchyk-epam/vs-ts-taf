...
| Config/env | `dotenv` |
| AI failure analysis | `@anthropic-ai/claude-agent-sdk` (`agents/` scripts — see [AI failure-analysis agents](#ai-failure-analysis-agents)) |
...
# Run only the visual (screenshot) suite
npx playwright test src/tests/screenShots.tests.ts
...
| `json` | A JSON report (`test-results/report.json` by default, or `JSON_REPORT_FILE`) — consumed by the `agents/` scripts below |
...
## AI failure-analysis agents

`agents/` contains two standalone Claude Agent SDK scripts (run with `tsx`, independent of Playwright) that read the first failing test from a `REPORTER=json` run and write a Markdown root-cause report to `AGENT_REPORT_DIR` (default `agent-reports/`). Both require `ANTHROPIC_API_KEY` in `.env`.

```powershell
# 1. Produce a JSON report to analyze
$env:REPORTER='json'; npx playwright test

# 2. Chain flow: a single agent summarizes the failure and proposes a root cause
npm run agent:chain

# 3. Orchestrator flow: an orchestrator delegates to collector/investigator/analyzer subagents
...
agents/
  chain/
    index.ts                      # Entry point: npm run agent:chain [path-to-json-report]
    testFailureAnalyzerAgent.ts   # Single-agent summary -> root cause
  orchestrator/
    index.ts                      # Entry point: npm run agent:orchestrator
    orchestratorAgent.ts          # Delegates to the subagents below
    subagents/                    # collector, codeInvestigator, analyzer
    tools/                        # Tools exposed to the agents (e.g. getFailure)
  reports/
    playwrightReport.ts           # Reads the Playwright JSON report, extracts the first failure
    reportGenerator.ts            # Writes the agent's run to a Markdown report
  shared/
    agent.ts                      # Shared agent configuration/helpers
```
...
| Variable | Purpose |
| --- | --- |
| `BASE_URL` | Base URL under test (default `https://cloud.google.com`) |
| `HEADLESS` | `true` runs the browser without a visible window |
| `LOG_LEVEL` | Winston log level: `trace`/`debug`/`info`/`warn`/`error` |
| `REPORTER` | Reporter to use — see [Reporting](#reporting) |
| `JSON_REPORT_FILE` | Output path for the JSON reporter (default `test-results/report.json`); also read by `agents/reports/playwrightReport.ts` |
...
| `ANTHROPIC_API_KEY` | API key used by the `agents/` failure-analysis scripts |
